# This file is no longer maintained, as the program was rewritten in C.
...
class DAWWindow(tk.Tk):
    def __init__(self):
        ...
        self.iconbitmap(os.path.join('Content', 'img', 'icons', 'novusblack.ico'))
        self.state('zoomed')
    ...
    def load_custom_font(self):
        font_path = os.path.join('Content', 'font', 'Inter', 'Inter-VariableFont_slnt,wght.ttf')
    ...
    def initUI(self):
        ...
        self.create_test_button("Most Used", 94, os.path.join('Content', 'img', 'icons', 'Explorer', 'mostused.png'))
        self.create_test_button("Favorites", 115, os.path.join('Content', 'img', 'icons', 'Explorer', 'favorites.png'))
        self.create_test_button("Recent Files", 136, os.path.join('Content', 'img', 'icons', 'Explorer', 'recentfiles.png'))
        ...
        for text, icon_file in library_buttons:
            icon_path = os.path.join('Content', 'img', 'icons', 'Explorer', icon_file)
    ...
    def display_files(self, folder_name):
        ...
        if not files:
            label = tk.Label(self.canvas, text="This folder is empty.", font=self.custom_font, bg='#A5A5A5', fg='#FFFFFF')
            label.place(x=file_explorer_x + 10, y=y_position + 10, anchor='nw')
        else:
            for file in files:
                file_path = os.path.join(folder_path, file)
                if file.lower().endswith(audio_extensions):
                    icon_path = os.path.join('Content', 'img', 'icons', 'Explorer', 'audiofile.png')
                else:
                    icon_path = None

                label = tk.Label(self.canvas, text=file, font=self.custom_font, bg='#A5A5A5', fg='#FFFFFF')
                label.place(x=file_explorer_x + 10, y=y_position, anchor='nw')
    ...
    def copy_folders_to_documents(self):
        source_dir = os.path.join(os.getcwd(), 'Content', 'App Preferences', 'Explorer', 'Explorer Folders')
        target_dir = os.path.join(os.path.expanduser('~'), 'Documents', 'Novus')

        status_file_path = os.path.join(os.getcwd(), 'App Preferences', 'Settings', 'CopyStatus.txt')
